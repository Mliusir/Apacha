...
let server = http.createServer((request, response) => {
  // 生成地址
  let targetPath = path.join(rootPath, querystring.unescape(request.url)); // console.log(targetPath);

  // 动态网站生成 index.nb
  if (request.url== '/index.mvp') {
    //创建数据连接
    ...
    connection.connect();
    // 查询数据
    connection.query('select * from manyhero', function (error,
      results,
      fields
    ) {
